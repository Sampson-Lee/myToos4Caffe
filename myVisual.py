...
# set corresponding to return certain data
def getvisual():
    ...
    filters = net.params[param][0].data   #提取某层参数（卷积核）
    filters = filters.transpose(0, 2, 3, 1)
    features = net.blobs[param].data[0]   #提取某层数据（特征），若没有前向传播特征图的值为0
    return filters,features
    
if __name__ == '__main__':
    filters,features=getvisual()
    vis_square(filters)
    print("\n")
    vis_square(features)
